...
export function Hero() {
  ...
  return (
    <section ref={ref} className="relative pt-36 pb-24 lg:pt-44 lg:pb-32 overflow-hidden">
      {/* aurora bg */}
      <motion.div
        style={{ x: sx, y: sy }}
      ...
      <div className="mx-auto max-w-7xl px-6 text-center">
        ...
        <motion.h1
          ...
          className="mt-8 text-4xl md:text-6xl lg:text-7xl font-semibold tracking-tight leading-[1.05]"
        >
        ...
        >
          Genesys Premier Partner helping enterprises modernize CCaaS, UCaaS, AI,
          Cloud, CRM and Customer Experience ecosystems — end-to-end.
        </motion.p>
        ...
        >
          <a href="#contact" className="group inline-flex items-center gap-2 px-6 py-3 rounded-full bg-aurora text-white font-medium shadow-glow hover:scale-[1.03] transition">
            Schedule Consultation
            <ArrowRight size={16} className="group-hover:translate-x-0.5 transition" />
          </a>
          <a href="#solutions" className="inline-flex items-center gap-2 px-6 py-3 rounded-full glass font-medium hover:bg-white/10 transition">
            Explore Solutions
          </a>
        </motion.div>

        {/* Hero visual: orbiting tech rings */}
        <motion.div
          initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.4, duration: 0.8 }}
          className="relative mx-auto mt-20 h-[420px] md:h-[520px] w-full max-w-3xl"
        >
          <OrbitingGlobe />
        </motion.div>
        ...
        <motion.div
          ...
          className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-px rounded-2xl overflow-hidden border-gradient bg-white/5"
        >
          {[
...
function OrbitingGlobe() {
  const techs = ["Genesys", "Salesforce", "AWS", "Azure", "GCP", "Cisco", "Avaya", "Zoom", "Oracle"];
  return (
    <div className="absolute inset-0 grid place-items-center">
      ...
      {[0.55, 0.75, 0.95].map((s, i) => (
        ...
        >
          {techs.slice(i * 3, i * 3 + 3).map((t, k) => {
            const angle = (k / 3) * 360;
            return (
              <div
                key={t}
                className="absolute top-1/2 left-1/2"
                style={{ transform: `rotate(${angle}deg) translate(0,-${s * 50}%)` }}
              >
                <motion.div
                  animate={{ rotate: i % 2 === 0 ? -360 : 360 }}
                  transition={{ duration: 40 + i * 10, repeat: Infinity, ease: "linear" }}
                  className="-translate-x-1/2 px-3 py-1.5 rounded-full glass text-[11px] font-medium whitespace-nowrap"
                >
                  {t}
                </motion.div>
              </div>
      ...
      {/* core */}
      <div className="relative h-44 w-44 rounded-full bg-aurora grid place-items-center shadow-glow">
        ...
        <div className="relative text-center">
          <div className="text-[10px] tracking-widest text-white/70">RACUBE</div>
          <div className="text-2xl font-black text-white">CX OS</div>
          <div className="text-[10px] tracking-widest text-white/70">PLATFORM</div>
        </div>
      </div>
    </div>
  );
        ...
        <div className="relative rounded-3xl overflow-hidden glass-strong p-10 md:p-16 border-gradient">
          ...
          <div className="grid lg:grid-cols-2 gap-12 relative">
            <div>
              ...
              <a href="#contact" className="mt-8 inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-aurora text-white font-medium shadow-glow">
                Explore open roles <ArrowRight size={14} />
              </a>
            </div>
            <div>
              <div className="text-xs uppercase tracking-widest text-muted-foreground mb-4">We're hiring expertise in</div>
              ...
              <div className="mt-8 grid grid-cols-3 gap-3 text-center">
                {[["Remote-first","Globally"], ["Top 1%","Engineering"], ["Real","Impact"]].map(([a,b])=>(
...
export function Eyebrow({ children }: { children: React.ReactNode }) {
  return (
    <div className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-[#1B75FF] font-semibold">
      <span className="h-px w-8 bg-[#1B75FF]" />{children}
    </div>
  );
}
